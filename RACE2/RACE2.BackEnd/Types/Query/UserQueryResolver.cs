...
namespace RACE2.BackEnd.Types
{
    ...
    public class UserQueryResolver
    {
        ...
        public async Task<IEnumerable<UserDetail>> GetUserDetails([Service] IUserService _userService)
        {
            try
            {
        ...
        public async Task<UserDetail> GetById([Service] IUserService _userService, int id)
        {
            return await _userService.GetUserById(id);
        }

        public async Task<UserSpecificDto> GetUserByEmailID([Service] IUserService _userService, string email)
        {
        ...
        public async Task<UserSpecificDto> GetUserWithRoles([Service] IUserService _userService, string email)
        {
            var result = await _userService.GetUserWithRoles(email);
            return result;
        }

        public async Task<UserDetail> MatchUserWithEmailAndPasswordHash([Service] IUserService _userService, string email, string passwordhash)
        {
            var result = await _userService.MatchUserWithEmailAndPasswordHash(email, passwordhash);
            return result;
        }

        public async Task<IntegrationResponseModel> GetEngineerReservoirByUUID([Service] IRACEIntegrationService _integrationService, string uuid)
        {
            var result = await _integrationService.GetEngineerReservoirByUUID(uuid);
            return result;
        }

        public async Task<OrganisationDTO> GetOrganisationAddressbyId([Service] IUserService _userService, int orgId)
        {
